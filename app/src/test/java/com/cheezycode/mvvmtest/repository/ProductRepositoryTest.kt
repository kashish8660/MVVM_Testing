...
class ProductRepositoryTest {
    ...
    fun testGetProducts_EmptyList() = runTest {
        Mockito.`when`(productsAPI.getProducts()).thenReturn(Response.success(emptyList())) //"success()" is a pre-defined function
    ...
    fun testGetProducts_expectedProductList() = runTest {
        ...
        Assert.assertEquals("Prod 1", result.data!![0].title) //result.data will return the whole list
    }
...
//"401" is the code and "Unauthorised" is the message that should be sent to user if the error comes
        val sut = ProductRepository(productsAPI)
        val result = sut.getProducts()
